...
def get_case_identifier(case):
    case_identifier = case[0].split("/")[-1].split(".nii")[0][:-5]
...
class ImageCropper(object):
    ...
    def load_crop_save(self, case, case_identifier, overwrite_existing=False):
        try:
            ...
            if overwrite_existing \
                ...
                data, seg, properties = self.crop_from_list_of_files(case[:-1], case[-1]) # 输入四个模态数据和一个mask，输出模态合一data和mask，数据property
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        list_of_args = []
        for j, case in enumerate(list_of_files):
            case_identifier = get_case_identifier(case) # case_identifier 数据名称  case 数据路径list
