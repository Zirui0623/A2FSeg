...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, subset_index_list: list, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        with context():
            with torch.no_grad():
                ...
                if use_sliding_window:
                    ...
                    res = self._internal_predict_3D_3Dconv_tiled(x, subset_index_list, step_size, do_mirroring, mirror_axes, patch_size,
                                                                 regions_class_order, use_gaussian, pad_border_mode,
                                                                 pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu,                                                                     verbose=verbose)
                else:
                    # res = self._internal_predict_3D_3Dconv(x, patch_size, do_mirroring, mirror_axes, regions_class_order,
                    res = self._internal_predict_3D_3Dconv(x, subset_index_list, patch_size, do_mirroring, mirror_axes, regions_class_order,
...
# endregion
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, subset_index_list: list, step_size: float, do_mirroring: bool, mirror_axes: tuple,
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        ...
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], subset_index_list, mirror_axes, do_mirroring,
    ...
    def _internal_predict_3D_3Dconv(self, x: np.ndarray, subset_index_list: list, min_size: Tuple[int, ...], do_mirroring: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        predicted_probabilities = self._internal_maybe_mirror_and_pred_3D(data[None], subset_index_list, mirror_axes, do_mirroring,
                                                                          None)[0]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], subset_index_list: list, mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            if m == 0:
                ...
                pred = self.inference_apply_nonlin(self(x, subset_index_list)[0])
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, ))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, )), subset_index_list)[0])
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, ))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, )), subset_index_list)[0])
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3)), subset_index_list)[0])
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (2, ))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (2, )), subset_index_list)[0])
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2)), subset_index_list)[0])
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2)), subset_index_list)[0])
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2)), subset_index_list)[0])
