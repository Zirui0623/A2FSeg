...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, subset_index_list: list, subset_size: int, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
            ...
            with torch.no_grad():
                if self.conv_op == nn.Conv3d:
                    if use_sliding_window:
                        ...
                        res = self._internal_predict_3D_3Dconv_tiled(x, subset_index_list, subset_size, step_size, do_mirroring, mirror_axes, patch_size,
                                                                     regions_class_order, use_gaussian, pad_border_mode,
                                                                     pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu,
                                                                     verbose=verbose)
                    else:
                        # res = self._internal_predict_3D_3Dconv(x, patch_size, do_mirroring, mirror_axes, regions_class_order,
                        res = self._internal_predict_3D_3Dconv(x, subset_index_list, subset_size, patch_size, do_mirroring, mirror_axes, regions_class_order,
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, subset_index_list: list, subset_size: int, step_size: float, do_mirroring: bool, mirror_axes: tuple,
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        ...
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], subset_index_list, subset_size, mirror_axes, do_mirroring,
    ...
    def _internal_predict_3D_3Dconv(self, x: np.ndarray, subset_index_list: list, subset_size: int, min_size: Tuple[int, ...], do_mirroring: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        predicted_probabilities = self._internal_maybe_mirror_and_pred_3D(data[None], subset_index_list, subset_size, mirror_axes, do_mirroring,
                                                                          None)[0]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], subset_index_list: list, subset_size: int, mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            if m == 0:
                ...
                pred = self.inference_apply_nonlin(self(x, subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, ))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, )), subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, ))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, )), subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3)), subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (2, ))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (2, )), subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2)), subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2)), subset_index_list, subset_size)[0])
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2))))
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2)), subset_index_list, subset_size)[0])
...
############################################################################################################
    def predict_3D_recon(self, num: int, x: np.ndarray, subset_index_list: list, subset_size: int, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                        use_sliding_window: bool = False,
                        step_size: float = 0.5, patch_size: Tuple[int, ...] = None, regions_class_order: Tuple[int, ...] = None,
                        use_gaussian: bool = False, pad_border_mode: str = "constant",
                        pad_kwargs: dict = None, all_in_gpu: bool = False,
            ...
            with context():
                with torch.no_grad():
                    ...
                    if use_sliding_window:
                        # res = self._internal_predict_3D_3Dconv_tiled(x, step_size, do_mirroring, mirror_axes, patch_size,
                        res = self._internal_predict_3D_3Dconv_tiled_recon(num, x, subset_index_list, subset_size, step_size, do_mirroring, mirror_axes, patch_size,
                                                                    regions_class_order, use_gaussian, pad_border_mode,
                                                                    pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu,
                                                                    verbose=verbose)
                    else:
                        # res = self._internal_predict_3D_3Dconv(x, patch_size, do_mirroring, mirror_axes, regions_class_order,
                        res = self._internal_predict_3D_3Dconv_recon(num, x, subset_index_list, subset_size, patch_size, do_mirroring, mirror_axes, regions_class_order,
                                                            pad_border_mode, pad_kwargs=pad_kwargs, verbose=verbose)
                    # else:
                    #     raise RuntimeError("Invalid conv op, cannot determine what dimensionality (2d/3d) the network is")
    ...
    def _internal_predict_3D_3Dconv_tiled_recon(self, num: int, x: np.ndarray, subset_index_list: list, subset_size: int, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                                patch_size: tuple, regions_class_order: tuple, use_gaussian: bool,
                                                pad_border_mode: str, pad_kwargs: dict, all_in_gpu: bool,
                                                verbose: bool) -> Tuple[np.ndarray, np.ndarray]:
        # better safe than sorry
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D_recon(
                        # data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                        num, data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], subset_index_list, subset_size, mirror_axes, do_mirroring,
                        gaussian_importance_map)[0]
        ...
        predicted_segmentation = aggregated_results[0]
        # if regions_class_order is None:
        #     predicted_segmentation = aggregated_results.argmax(0)
        # else:
        #     if all_in_gpu:
    ...
    def _internal_predict_3D_3Dconv_recon(self, num: int, x: np.ndarray, subset_index_list: list, subset_size: int, min_size: Tuple[int, ...], do_mirroring: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        # predicted_probabilities = self._internal_maybe_mirror_and_pred_3D(data[None], mirror_axes, do_mirroring,
        predicted_probabilities = self._internal_maybe_mirror_and_pred_3D_recon(num, data[None], subset_index_list, subset_size, mirror_axes, do_mirroring,
                                                                          None)[0]
    ...
    # def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
    def _internal_maybe_mirror_and_pred_3D_recon(self, num: int, x: Union[np.ndarray, torch.tensor], subset_index_list: list, subset_size: int, mirror_axes: tuple,
                                           do_mirroring: bool = True,
                                           mult: np.ndarray or torch.tensor = None) -> torch.tensor:
        assert len(x.shape) == 5, 'x must be (b, c, x, y, z)'
        ...
        x = maybe_to_torch(x)
        result_torch = torch.zeros([1, 1] + list(x.shape[2:]),
                                   dtype=torch.float)
        ...
        for m in range(mirror_idx):
            if m == 0:
                # pred = self.inference_apply_nonlin(self(x))
                pred = self(x, subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, ))))
                pred = self(torch.flip(x, (4, )), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, ))))
                pred = self(torch.flip(x, (3, )), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3))))
                pred = self(torch.flip(x, (4, 3)), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (2, ))))
                pred = self(torch.flip(x, (2, )), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2))))
                pred = self(torch.flip(x, (4, 2)), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2))))
                pred = self(torch.flip(x, (3, 2)), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2))))
                pred = self(torch.flip(x, (4, 3, 2)), subset_index_list)[-1][:,num][:,16:17]
                result_torch += 1 / num_results * torch.flip(pred, (4, 3, 2))
