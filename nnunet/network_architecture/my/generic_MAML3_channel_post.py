...
from nnunet.network_architecture.my.neural_network3 import SegmentationNetwork
...
class Generic_MAML_multi3_channel_post(SegmentationNetwork):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def __init__(self, modality_num, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        Questions? -> f.isensee@dkfz.de
        """
        super(Generic_MAML_multi3_channel_post, self).__init__()
        self.inference_apply_nonlin = lambda x: x  # softmax_helper
        self._gaussian_3d = self._patch_size_for_gaussian_3d = None
        self._gaussian_2d = self._patch_size_for_gaussian_2d = None
        ...
        for i in range(modality_num):
            self.modality_specific_models.append(Generic_UNet(1, base_num_features, num_classes, num_pool, num_conv_per_stage, 
                 ...
                 seg_output_use_bias))
        '''
        self.fusion_decoder = Generic_UNet_Decoder(1, base_num_features, num_classes, num_pool, num_conv_per_stage, 
                 feat_map_mul_on_downscale, conv_op,
                 norm_op, norm_op_kwargs,
                 dropout_op, dropout_op_kwargs,
                 nonlin, nonlin_kwargs, deep_supervision, dropout_in_localization,
...
##############################################################################
    ...
    def forward(self, x, subset_idx_list=[14], num = 4):
...
##############################################################################
            ...
            if drop[i]:
                # modality_features.append(0)
                continue

            ms_outputs, feature = self.modality_specific_models[i](x[i])
...
##############################################################################
        ...
        '''
        ms_outputs = self.fusion_decoder(fusion_feature)
        modality_features.append(ms_outputs[0])
        final_outputs += ms_outputs[1:]
        '''
        #add
        modality_features.append(torch.mean(torch.stack(modality_features, 0), 0))
        #
##############################################################################
...
##############################################################################
            if drop[i]:
                # attention_maps.append(0)
                continue
##############################################################################
            attention_maps.append(self.modality_aware_modules[i](torch.cat([modality_features[t], modality_features[-1]], dim=1)))
            t = t + 1
...
##############################################################################
        ...
        output = self.lastconv[0](torch.cat((output, modality_features[-1]), dim = 1))
##############################################################################

        output = self.output(output)

        final_outputs = [output, ] + final_outputs
