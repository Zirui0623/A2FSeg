...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    ...
    for subdir in ["imagesTr", "imagesTs"]:
        ...
        nii_files = [join(curr_dir, i) for i in os.listdir(curr_dir) if i.endswith(".nii")]
        nii_files.sort()
        for n in nii_files:
            files.append(n)
...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    for val in val_files:
        cur_pat = []
        for mod in range(num_modalities):
            cur_pat.append(join(base_folder_splitted, "imagesVal", val['image'].split("/")[-1][:-4] +
                                "_%04.0d.nii" % mod))
        cur_pat.append(join(base_folder_splitted, "labelsVal", val['label'].split("/")[-1]))
        lists.append(cur_pat)

    for ts in test_files:
        cur_pat = []
        for mod in range(num_modalities):
            cur_pat.append(join(base_folder_splitted, "imagesTs", ts['image'].split("/")[-1][:-4] +
                                "_%04.0d.nii" % mod))
        cur_pat.append(join(base_folder_splitted, "labelsTs", ts['label'].split("/")[-1]))
        lists.append(cur_pat)
...
def create_lists_from_splitted_dataset_folder(folder):
    ...
    list_of_lists = []
    for f in caseIDs:
        list_of_lists.append(subfiles(folder, prefix=f, suffix=".nii", join=True, sort=True))
    return list_of_lists


def get_caseIDs_from_splitted_dataset_folder(folder):
    files = subfiles(folder, suffix=".nii", join=False)
