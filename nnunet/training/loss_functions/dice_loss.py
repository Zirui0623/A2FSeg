...
class DC_and_BCE_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        return result
############################################################
class DC_and_BCE_loss2(nn.Module):
    def __init__(self, bce_kwargs, soft_dice_kwargs, aggregate="sum"):
        """
        DO NOT APPLY NONLINEARITY IN YOUR NETWORK!
        ...
        :param aggregate:
        """
        super(DC_and_BCE_loss2, self).__init__()
    ...
    def forward(self, net_output, target):
        ...
        soft_target = torch.sigmoid(target)
        hard_target = (soft_target > 0.5).float()

        ce_loss = self.ce(net_output, hard_target)
        dc_loss = self.dc(net_output, soft_target)
