...
def get_case_identifiers_from_raw_folder(folder):
    case_identifiers = np.unique(
        [i[:-9] for i in os.listdir(folder) if i.endswith(".nii") and (i.find("segFromPrevStage") == -1)])
    return case_identifiers
...
#####################################################################################
def get_case_identifiers_from_raw_folder_label(folder):
    case_identifiers = np.unique(
        [i[:-4] for i in os.listdir(folder) if i.endswith(".nii") and (i.find("segFromPrevStage") == -1)])
    return case_identifiers

def load_dataset_each(tr_folder, val_folder, ts_folder, folder, num_cases_properties_loading_threshold=1000):
    # we don't load the actual data but instead return the filename to the np file.
    print('loading dataset')
    tr_case_identifiers = get_case_identifiers_from_raw_folder_label(tr_folder)
    val_case_identifiers = get_case_identifiers_from_raw_folder_label(val_folder)
    ts_case_identifiers = get_case_identifiers_from_raw_folder_label(ts_folder)

    tr_case_identifiers.sort()
    val_case_identifiers.sort()
    ts_case_identifiers.sort()

    dataset_tr = OrderedDict()
    dataset_val = OrderedDict()
    dataset_ts = OrderedDict()
#----------------train------------------
    for c in tr_case_identifiers:
        dataset_tr[c] = OrderedDict()
        dataset_tr[c]['data_file'] = join(folder, "%s.npz" % c)

        # dataset[c]['properties'] = load_pickle(join(folder, "%s.pkl" % c))
        dataset_tr[c]['properties_file'] = join(folder, "%s.pkl" % c)

        if dataset_tr[c].get('seg_from_prev_stage_file') is not None:
            dataset_tr[c]['seg_from_prev_stage_file'] = join(folder, "%s_segs.npz" % c)

    if len(tr_case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading all case properties')
        for i in dataset_tr.keys():
            dataset_tr[i]['properties'] = load_pickle(dataset_tr[i]['properties_file'])

#----------------val------------------
    for c in val_case_identifiers:
        dataset_val[c] = OrderedDict()
        dataset_val[c]['data_file'] = join(folder, "%s.npz" % c)

        # dataset[c]['properties'] = load_pickle(join(folder, "%s.pkl" % c))
        dataset_val[c]['properties_file'] = join(folder, "%s.pkl" % c)

        if dataset_val[c].get('seg_from_prev_stage_file') is not None:
            dataset_val[c]['seg_from_prev_stage_file'] = join(folder, "%s_segs.npz" % c)

    if len(val_case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading all case properties')
        for i in dataset_val.keys():
            dataset_val[i]['properties'] = load_pickle(dataset_val[i]['properties_file'])

#----------------test------------------
    for c in ts_case_identifiers:
        dataset_ts[c] = OrderedDict()
        dataset_ts[c]['data_file'] = join(folder, "%s.npz" % c)

        # dataset[c]['properties'] = load_pickle(join(folder, "%s.pkl" % c))
        dataset_ts[c]['properties_file'] = join(folder, "%s.pkl" % c)

        if dataset_ts[c].get('seg_from_prev_stage_file') is not None:
            dataset_ts[c]['seg_from_prev_stage_file'] = join(folder, "%s_segs.npz" % c)

    if len(ts_case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading all case properties')
        for i in dataset_ts.keys():
            dataset_ts[i]['properties'] = load_pickle(dataset_ts[i]['properties_file'])
