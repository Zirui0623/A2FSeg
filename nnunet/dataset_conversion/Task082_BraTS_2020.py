...
if __name__ == "__main__":
    ...
    downloaded_data_dir = "/data/zirui/lab/data/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/"
    # downloaded_data_dir_val = "/home/fabian/Downloads/MICCAI_BraTS2020_ValidationData"
    # downloaded_data_dir_val = "/data/zirui/lab/data/BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData"
...
#########################################################
    ...
    trainset_size = 0.7
    valset_size = 0.1
    # paths = glob(downloaded_data_dir)
    paths = os.listdir(downloaded_data_dir)
    train_paths, val_paths = train_test_split(paths, test_size=1-trainset_size, random_state=1)

    val_ratio = valset_size / (1 - trainset_size)

    val_paths, test_paths = train_test_split(val_paths, test_size=1-val_ratio, random_state=1)
## trainset
    train_patient_names = []
    for p in train_paths:
        t1 = join(downloaded_data_dir, p+'/'+p+'_t1.nii')
        t1c = join(downloaded_data_dir, p+'/'+p+'_t1ce.nii')
        t2 = join(downloaded_data_dir, p+'/'+p+'_t2.nii')
        flair = join(downloaded_data_dir, p+'/'+p+'_flair.nii')
        seg = join(downloaded_data_dir, p+'/'+p+'_seg.nii')
        
        # print(t1, t1c, t2, flair, seg)
        train_patient_names.append(p)

        shutil.copy(t1, join(target_imagesTr, p + "_0000.nii"))
        shutil.copy(t1c, join(target_imagesTr, p + "_0001.nii"))
        shutil.copy(t2, join(target_imagesTr, p + "_0002.nii"))
        shutil.copy(flair, join(target_imagesTr, p + "_0003.nii"))

        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, p + ".nii"))
## valset
    val_patient_names = []
    for p in val_paths:
        t1 = join(downloaded_data_dir, p+'/'+p+'_t1.nii')
        ...
        shutil.copy(t1, join(target_imagesVal, p + "_0000.nii"))
        shutil.copy(t1c, join(target_imagesVal, p + "_0001.nii"))
        shutil.copy(t2, join(target_imagesVal, p + "_0002.nii"))
        shutil.copy(flair, join(target_imagesVal, p + "_0003.nii"))

        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsVal, p + ".nii"))
## tsetset
    test_patient_names = []
    for p in test_paths:
        t1 = join(downloaded_data_dir, p+'/'+p+'_t1.nii')
        ...
        shutil.copy(t1, join(target_imagesTs, p + "_0000.nii"))
        shutil.copy(t1c, join(target_imagesTs, p + "_0001.nii"))
        shutil.copy(t2, join(target_imagesTs, p + "_0002.nii"))
        shutil.copy(flair, join(target_imagesTs, p + "_0003.nii"))

        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTs, p + ".nii"))
#########################################################
# region
...
#             isfile(flair),
#             isfile(seg)
#         ]), "%s" % train_patient_name

#         shutil.copy(t1, join(target_imagesTr, train_patient_name + "_0000.nii.gz"))
#         shutil.copy(t1c, join(target_imagesTr, train_patient_name + "_0001.nii.gz"))
#         shutil.copy(t2, join(target_imagesTr, train_patient_name + "_0002.nii.gz"))
#         shutil.copy(flair, join(target_imagesTr, train_patient_name + "_0003.nii.gz"))

#         copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, train_patient_name + ".nii.gz"))
# endregion
    ...
    json_dict['numTraining'] = len(train_paths)
    json_dict['numVal'] = len(val_paths)
    json_dict['numTest'] = len(test_paths)
    json_dict['training'] = [{'image': "./imagesTr/%s.nii" % i, "label": "./labelsTr/%s.nii" % i} for i in
                             train_patient_names]
    json_dict['val'] = [{'image': "./imageVal/%s.nii" % i, "label": "./labelsVal/%s.nii" % i} for i in
                             val_patient_names]
    json_dict['test'] = [{'image': "./imagesTs/%s.nii" % i, "label": "./labelsTs/%s.nii" % i} for i in
                             test_patient_names]
