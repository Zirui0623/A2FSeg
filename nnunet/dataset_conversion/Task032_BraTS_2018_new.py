...
## trainset
    ...
    for p in train_paths:
        t1 = join(downloaded_data_dir, p+'/'+p+'_t1.nii.gz')
        t1c = join(downloaded_data_dir, p+'/'+p+'_t1ce.nii.gz')
        t2 = join(downloaded_data_dir, p+'/'+p+'_t2.nii.gz')
        flair = join(downloaded_data_dir, p+'/'+p+'_flair.nii.gz')
        seg = join(downloaded_data_dir, p+'/'+p+'_seg.nii.gz')
...
## valset
    ...
    for p in val_paths:
        ...
        shutil.copy(t1, join(target_imagesVal, p + "_0000.nii.gz"))
        shutil.copy(t1c, join(target_imagesVal, p + "_0001.nii.gz"))
        shutil.copy(t2, join(target_imagesVal, p + "_0002.nii.gz"))
        shutil.copy(flair, join(target_imagesVal, p + "_0003.nii.gz"))

        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsVal, p + ".nii.gz"))
## tsetset
    test_patient_names = []
    for p in test_paths:
        t1 = join(downloaded_data_dir, p+'/'+p+'_t1.nii.gz')
...
# endregion
    ...
    json_dict['training'] = [{'image': "./imagesTr/%s.nii" % i, "label": "./labelsTr/%s.nii.gz" % i} for i in
                             train_patient_names]
    json_dict['val'] = [{'image': "./imageVal/%s.nii" % i, "label": "./labelsVal/%s.nii.gz" % i} for i in
                             val_patient_names]
    json_dict['test'] = [{'image': "./imagesTs/%s.nii" % i, "label": "./labelsTs/%s.nii.gz" % i} for i in
                             test_patient_names]
